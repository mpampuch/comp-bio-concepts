...
cleaned_sentence_2 = map(lowercase, messy_sentence_2) |> filter(isinalphabet, )
...
function clean(text)
	return text |> unaccent |> lowercase |> filter(isinalphabet, )
end
...
# ╔═╡ c937c42f-6cc0-40e7-ac5b-42927be629d5
alphabet
...
unused_letters = filter(letter -> 
						sample_freqs[index_of_letter(letter)] |> iszero,
						alphabet
					   )
...
double_letters = [ letter for (i, letter) in enumerate(alphabet) if sample_freq_matrix[i, i] |> !iszero ]
...
most_likely_to_follow_w = [sample_freq_matrix[index_of_letter('w'), j] for j in eachindex(alphabet)] |> # Get all the frequences of all combinations following w
	argmax |> # Get the index of the value with the highest value
	x -> alphabet[x] # Retrieve the letter of the alphabet 
...
most_likely_to_precede_w = [sample_freq_matrix[i, index_of_letter('w')] for i in eachindex(alphabet)] |> # Get all the frequences of all combinations preceding w
	argmax |> # Get the index of the value with the highest value
	x -> alphabet[x] # Retrieve the letter of the alphabet 
...
# col_sums = [sum(sample_freq_matrix[:, j]) for j in 1:size(sample_freq_matrix, 2)]
col_sums = sum(sample_freq_matrix, dims=1)

# ╔═╡ cc62929e-f9af-11ea-06b9-439ac08dcb52
row_col_answer = md"""
A higher row sum → this letter is more common as a starting point for transitions.
	
A row sum of 0 → this letter never appears as a starting point.
- row_sums[1] = 0.063 → the first letter makes up 6.3% of all transitions in the sample.
- row_sums[27] = 0.164 → the 27th letter (space) is very frequent as a starting letter.

A higher column sum → this letter is more commonly reached as a next symbol.
	
A column sum of 0 → this letter never appears as a destination.

- row_sums[5] = 0.1066 → 5th letter starts ~10.7% of all transitions.
- col_sums[1] = 0.0624 → 1st letter is the destination for ~6.2% of all transitions.
...
function matrix_distance(A, B)
	...
	# Substact the corresponding elements 
	C = A - B
...
function ngrams(words, n)
	...
	return map(starting_positions) do i
		words[i:i + n - 1]
	end
end
...
emma_count = 
	emma |> 
		x -> replace(x, r"[^\w\s]" => " ") |> # Replace anything that isn't a word character or a whitespace with a whitespace
		x -> split(x) |> 
		x -> word_counts(x)["Emma"]
...
function completion_cache(grams)
	...
	for gram in grams 
		first_grams = gram[begin:end - 1]
		last_gram = [gram[end]]
		if ! haskey(cache, first_grams)
			cache[first_grams] = []
		end
		append!(cache[first_grams],last_gram)
...
# ╔═╡ 551dfefd-bb22-429c-886d-20a4bd4eb849
...
manifest_format = "2.0"
project_hash = "2575db997a6bdc4cc12534e662991f13ab454352"
