...
begin
    ...
    # Activate your scratch environment
    Pkg.activate("~/.julia/scratchspaces/c3e4b0f8-55cb-11ea-2926-15256bba5781/pkg_envs/env_obubbpgtue")
    ...
    Pkg.add("Ueauty")
    Pkg.add(Pkg.PackageSpec(name="JET", version="0.11"))
    
    # Precompile everything
    Pkg.precompile()
end

# ╔═╡ ce1a7fd7-7b01-4e50-a6f8-5530cf5ba48e
begin
...
# ╔═╡ a151e470-2922-4c52-a4dd-ad40ebc8d31a
colored_line(v)

# ╔═╡ 613ccbe8-e860-4632-afff-da12ef99b687
@bind ℓ_box Slider(1:10, show_value=true, default=10)
...
# ╔═╡ bb733964-e14c-44d9-82ea-ac67ff8a00c8
function convolve(v::AbstractVector, k::AbstractVector)
	ℓ = (length(k) - 1) ÷ 2
	n = length(v)

	# Create an Offset Array that's length(v) + 2ℓ in size
	v_extended = OffsetArray(
		zeros(eltype(v), (n + 2ℓ)),
		-ℓ:(n + ℓ - 1) # Offset the indicies to start at -ℓ and end at (n + ℓ - 1) 
	)

	# Fill in the middle of the extended vector with the regular vector contents
	v_extended[0:(n - 1)] .= v # Assigns v to v_extended range (using a view), element-wise

	# Fill in (clamp) all the left-most padded values with the left-most value of v
	v_extended[-ℓ:-1] .= v[begin]

	# Fill in (clamp) all the right-most padded values with the right-most value of v
	v_extended[n:end] .= v[end]

	# Prepare result vector
	v′ = similar(v, Number)

	for i in eachindex(v)
		v_extendedᵢ = ∑(v_extended[(i - ℓ - 1):(i - 1 + ℓ)] .* k)
		v′[i] = v_extendedᵢ
	end
...
# ╔═╡ 589e7de5-901d-4d9d-bcd6-3bee25719e51
begin
	n2 = 2 * 2 + 1
	fill(1 / n2, n2)
...
function box_blur_kernel(ℓ)
	N = 2ℓ + 1 # Size of k
	k = fill(1 / N, N) # Each element of k is weighted equally. Multiplying vᵢ₋ₘ by kₘ when kₘ = 1 / N is equivalent to taking to considering vᵢ₋ₘ the numerator and kₘ the denominator in a mean calculation
	return k
end
...
function box_blur(v::AbstractArray, ℓ)
	v′ = similar(v, Number)
	for i in eachindex(v)
		window_array = Float32[]
		for j in i-ℓ:i+ℓ
			val = extend(v, j)
			append!(window_array, val)
		end
		v′[i] = mean(window_array)
	end
	return v′
end
...
# ╔═╡ c8048026-2541-436b-84c7-afa923aa30be
collect(Iterators.product(-1:7,-1:7).iterators[1])
...
function convolve(M::AbstractMatrix, K::AbstractMatrix)
	...
	# # 
	# Extend the original matrix by half the length of K and half the width of K, both rounded down to the nearest integer (floored)
	# # 

	M_w = size(M, 1) 	# Matrix width
	K_w = size(K, 1) 	# Kernel width
	Mₗ = size(M, 2) 	# Matrix length
	Kₗ = size(K, 2) 	# Kernel length
	
	k = K_w ÷ 2 	# Half kernel width
	ℓ = Kₗ ÷ 2 		# Half kernel length

	# Get the max and min values for the new range of the extended matrix
	iₘᵢₙ = 1 - k
	iₘₐₓ = M_w + k
	jₘᵢₙ = 1 - ℓ
	jₘₐₓ = Mₗ + ℓ

	# Create a new Matrix that's extended by half the kernel size
	M_extended = OffsetMatrix(
		[extend(M, i, j) for (i, j) in Iterators.product(
			iₘᵢₙ:iₘₐₓ, 
			jₘᵢₙ:jₘₐₓ)
		],
		iₘᵢₙ:iₘₐₓ,
	...
	# #
	for (i, rowᵢ) in enumerate(eachrow(M))
		for (j, _) in enumerate(rowᵢ)
			Mᵢⱼ = ∑(M_extended[
				(i - k):(i + k),
				(j - ℓ):(j + ℓ)	
			] .* K) # Multiply by subsetted values by Kernel
			
			# Add summed result to transformed matrix
			M′[i,j] = Mᵢⱼ
		end
	end
...
gauss(x::Real, y::Real; σ=1) = 2π*σ^2 * gauss(x; σ=σ) * gauss(y; σ=σ)

# ╔═╡ 1c8b4658-ee0c-11ea-2ede-9b9ed7d3125e
function gaussian_kernel_1D(n; σ = 1)
	k = [gauss(x, σ = σ) for x in -n:n] # Sample each pixel
	k ./= sum(k) # Divide every element in k by the total sum to normalize
	return k
end
...
function with_gaussian_blur(image; σ=3, ℓ=5)
	K = [gauss(x, y, σ = σ) for (x, y) in Iterators.product(-ℓ:ℓ, -ℓ:ℓ)] # Create 2ℓ+1 x 2ℓ+1 Gaussian Kernel 
	K ./= sum(K) # Normalize the Kernel so all values added = 1
...
function with_sobel_edge_detect(image)
	...
	convolveₓ = convolve(image, Gₓ)
	convolve_y = convolve(image, G_y)
	image′ = convolveₓ + convolve_y
	return image′
end
...
with_gaussian_blur(gauss_camera_image; σ=face_σ, ℓ=face_l)
